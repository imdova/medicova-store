...
export interface FlashSaleProduct {
  id: string;
  title: LocalizedTitle; // simplified, instead of LocalizedTitle
  price: number; // original price
  del_price?: number; // discounted price
...
const flashSaleSchema = z.object({
  ...
  status: z.enum(["published", "draft", "expired"]),
  createdAt: z.string().min(1, "Start date is required"),
...
export default function EditFlashSalePage() {
  ...
  // Find the flash sale by slug
  const currentFlashSale = FlashSalesData.find((sale) => sale.id === slug);
  ...
  useEffect(() => {
    if (currentFlashSale) {
      form.reset({
        ...
        createdAt: currentFlashSale.createdAt ?? "",
        endDate: currentFlashSale.endDate ?? "",
        products: currentFlashSale.products ?? [],
      });
      setSelectedProducts(currentFlashSale.products ?? []);
    }
    setIsLoading(false);
  }, [currentFlashSale]);
  ...
  const onSubmit = async (data: FlashSaleFormData) => {
    try {
      ...
      console.log("Updated Flash Sale Data:", formData);
  ...
  const t = {
    ...
    ar: {
      ...
      required: "مطلوب",
      save: "تحديث",
      saveExit: "تحديث وخروج",
      add_product: "إضافة منتج",
      remove: "إزالة",
      edit_flash_sale: "تحرير العرض الترويجي",
      english: "الإنجليزية",
      arabic: "العربية",
      no_products_found: "لم يتم العثور على منتجات",
      select_products: "اختر المنتجات لإضافتها للعرض الترويجي",
      not_found: "العرض الترويجي غير موجود",
  ...
  return (
    <div>
      {/* Header */}
      <div className="mb-6">
        <h1 className="text-2xl font-bold">{t.edit_flash_sale}</h1>
        <div className="mt-2 text-sm text-primary">
          {language === "en" ? "Editing:" : "جاري تحرير:"}{" "}
          {currentFlashSale.name[language]}
        </div>
      </div>
              ...
              <Card className="p-6">
                <div className="mb-4">
                  ...
                  <p className="text-sm text-gray-600">
                    ...
                    {language === "en" ? "products selected" : "منتج مختار"}
                  </p>
                </div>
