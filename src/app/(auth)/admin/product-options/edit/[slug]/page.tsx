...
const optionSchema = z.object({
  id: z.string().optional(), // if you allow auto-generated IDs
  slug: z.string().optional(),
  name: z.object({
    en: z.string().min(1, messages.name_en_required.en),
    ar: z.string().min(1, messages.name_ar_required.ar),
  }),
  option_type: z.enum(["dropdown", "checkbox", "radio", "text"]),
  option_values: z
    .array(
      z.object({
        id: z.string(),
  ...
  isRequired: z.boolean(), // new field for backward compatibility
  createdAt: z.string().datetime(), // ISO date-time string
});
...
export default function EditOptionPage() {
  ...
  const [currentOption, setCurrentOption] = useState<
    (typeof ProductOptions)[0] | null
  >(null);

  // Load option data based on slug
  useEffect(() => {
    const loadOptionData = () => {
      ...
      setTimeout(() => {
        const option = ProductOptions.find((opt) => opt.id === slug);
  ...
  const onSubmit = async (data: OptionFormData) => {
    try {
      const formData = { ...data, option_values: optionValues };
      console.log("Updating option:", formData);
  ...
  const t = {
    en: {
      ...
      published: "Update Option",
      add_option_value: "Add Option Value",
      remove: "Remove",
      edit_option: "Edit Product Option",
      english: "English",
      arabic: "Arabic",
    ...
    ar: {
      ...
      save: "حفظ",
      saveExit: "حفظ وخروج",
      published: "تحديث الخيار",
      add_option_value: "إضافة قيمة خيار",
      remove: "إزالة",
      edit_option: "تحرير خيار المنتج",
      english: "الإنجليزية",
      arabic: "العربية",
      loading: "جاري تحميل الخيار...",
      not_found: "الخيار غير موجود",
    },
  }[language];
  ...
  return (
    <div>
      {/* Header */}
      <div className="mb-6">
        <h1 className="text-2xl font-bold">{t.edit_option}</h1>
        <div className="mt-2 text-sm text-gray-500">
          Editing: {currentOption.name[language]} (ID: {currentOption.id})
        </div>
      </div>
